...
if not FLICKR_API_KEY or not FLICKR_API_SECRET:
    raise ValueError(
        "API keys are not defined. Please check your .env file and the custom path.")
root = tk.Tk()
...
def validate_api_keys():
    try:
        flickr = FlickrAPI(FLICKR_API_KEY, FLICKR_API_SECRET,
                           format='parsed-json')
        # Use an actual API endpoint for validation
        response = flickr.photos.getInfo(photo_id='1')
        if response['stat'] == 'ok':
        ...
        print("Error validating API keys:", e)
        root.quit()
...
def download_images_from_flickr():
    ...
    if not folder_selected:
        messagebox.showwarning("Folder Not Selected",
                               "Please select a folder to save the images.")
        return

    if not search_term:
        messagebox.showwarning("Search Term Not Provided",
                               "Please provide a search term.")
        return

    if not num_of_images:
        messagebox.showwarning("Number of Images Not Provided",
                               "Please provide the number of images to download.")
        return
    ...
    # Modify the parameters to search for Creative Commons licensed photos
    photos = flickr.photos.search(
        text=search_term, per_page=num_of_images, page=1, sort='relevance',
        license='1,2,3,4,5,6',  # This includes various Creative Commons licenses
    ...
    if photos and photos['photos']['photo']:
        ...
        for photo in photos_list:
            ...
            download_queue.put((url, folder_selected))
...
def download_image():
    while not download_queue.empty():
        try:
            url, folder = download_queue.get_nowait()
        except Empty:
            break
        else:
            global serial_number
            with lock:
                serial_number += 1
                image_num = serial_number
            response = requests.get(url)
            if response.status_code == 200:
                with open(f"{folder}/image_{image_num}.jpg", "wb") as file:
                    file.write(response.content)
                with lock:
                    gui_queue.put_nowait(image_num)
...
def check_gui_queue():
    while True:
        try:
            image_num = gui_queue.get_nowait()
        except Empty:
            break
        else:
...
download_btn = ttk.Button(root, text="Download",
                          command=download_images_from_flickr)
